...
#         print("Specificity Standard Deviation: {}".format(np.std(avg_spec)))
    ...
    auc_CI=st.t.interval(0.95, len(all_auc)-1, loc=np.nanmean(all_auc), scale=np.nanstd(all_auc)/math.sqrt(len(all_acc)))
    import os
...
#filename='combine_MConly_wo_dropouts_4_mor+conn'
filename='combine_MConly_wo_dropouts_0_biomarker'
...
'RandomForestClassifier': [
     { 'RandomForestClassifier__n_estimators': np.arange(10, 500, 50) },
...
'linear_model.LogisticRegression': [
     {'linear_model.LogisticRegression__C':[0.001, 0.01, 0.1, 1, 10]},
