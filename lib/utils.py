...
class model_perf(object):
    ...
    def __init__(s):
        ...
        s.fit_accuracies, s.fit_losses, s.fit_time, s.fit_scores = {}, {}, {}, {}
        s.train_accuracy, s.train_f1, s.train_loss, s.train_score = {}, {}, {}, {}
        s.test_accuracy, s.test_f1, s.test_loss, s.test_score = {}, {}, {}, {}
        s.y_labels, s.n_bin = [], []

    def test(s, model, name, params, train_data, train_labels, val_data, val_labels, test_data, test_labels, bin_ix):
        s.y_labels = test_labels
        s.n_bin = bin_ix
        s.params[name] = params
        s.fit_accuracies[name], s.fit_losses[name], s.fit_time[name], s.fit_scores[name] = model.fit(train_data, 
                                                                                                     train_labels, val_data, val_labels)
        string, s.train_accuracy[name], s.train_f1[name], s.train_loss[name], s.train_score[name] = model.evaluate(train_data,
                                                                                                                   train_labels)
        print('train {}'.format(string))
        string, s.test_accuracy[name], s.test_f1[name], s.test_loss[name], s.test_score[name] = model.evaluate(test_data,
                                                                                                               test_labels)
        print('test  {}'.format(string))
        s.names.add(name)
    ...
    def show(s, fontsize=None):
        ...
        for name in sorted(s.names):
            ...
            #ax[0].plot(steps, s.fit_accuracies[name], lw=3, ls='-', label=name)
            #ax[1].plot(steps, s.fit_losses[name], lw=3, ls='-', label=name)
            #print("Scores: {}".format(s.test_score[name][:,1]))
            #print("Test Lables: {}".format(s.y_labels))
            test_acc = s.test_accuracy[name]
        '''
        ...
        ax[0].set_title('Fold {} Validation Accuracy'.format(s.n_bin))
        ax[1].set_title('Fold {} Training Loss'.format(s.n_bin))
        ax[0].legend(loc='lower right')
        ax[1].legend(loc='upper right')
        fig.savefig('training-{}-strat.eps'.format(s.n_bin))
        plt.show()
        '''

        return s.test_score['CGCNN'][:,1], s.test_f1['CGCNN'], s.y_labels, test_acc, 
             
    def roc(s, labels, scores, n_feat):
        fpr = dict()
        tpr = dict()
        roc_auc = dict()
        thresholds = dict()
        plt.figure()
        fpr[0], tpr[0], thresholds[0] = roc_curve(labels, scores)
        roc_auc[0] = auc(fpr[0], tpr[0])
        plt.plot(fpr[0], tpr[0], lw=3, label='area = {:0.2f}'.format(roc_auc[0]))
        ...
        plt.savefig('ROC-{}-strat.eps'.format(n_feat))
        plt.show()
